    ...
    if 'data_root' in conf:
        if 'inbalenceDataSets' in conf:
            if (conf['inbalenceDataSets']['imbalance_sub_dataset']):
                ...
                train_loader_a.append(
                    get_data_loader_folder(os.path.join(conf['data_root'], 'trainA'), batch_size, True,
                ...
                test_loader_a.append(get_data_loader_folder(os.path.join(conf['data_root'], 'testA'), batch_size, False,
                                                            ...
                                                            ratio_1_to_2=conf['inbalenceDataSets']['ratio_A_1_to_2']))
                train_loader_b.append(
                    get_data_loader_folder(os.path.join(conf['data_root'], 'trainB'), batch_size, True,
                ...
                test_loader_b.append(get_data_loader_folder(os.path.join(conf['data_root'], 'testB'), batch_size, False,
                                                            ...
                                                            ratio_1_to_2=conf['inbalenceDataSets']['ratio_B_1_to_2']))
            else:
                train_loader_a.append(get_data_loader_folder(os.path.join(conf['data_root'], 'trainA'),
                                                             batch_size, True, new_size_a, height, width, num_workers,
                                                             True, config=conf, is_data_A=True))
                test_loader_a.append(get_data_loader_folder(os.path.join(conf['data_root'], 'testA'),
                                                            batch_size, False, new_size_a, new_size_a, new_size_a,
                                                            num_workers, True, config=conf, is_data_A=True))
                train_loader_b.append(get_data_loader_folder(os.path.join(conf['data_root'], 'trainB'),
                                                             batch_size, True, new_size_b, height, width, num_workers,
                                                             True, config=conf, is_data_A=False))
                test_loader_b.append(get_data_loader_folder(os.path.join(conf['data_root'], 'testB'),
                                                            batch_size, False, new_size_b, new_size_b, new_size_b,
                                                            num_workers, True, config=conf, is_data_A=False))
        else:
        ...
        train_loader_a.append(
            get_data_loader_list(conf['data_folder_train_a'], conf['data_list_train_a'], batch_size, True,
                                 new_size_a, height, width, num_workers, True, is_data_A=True))
        test_loader_a.append(
            get_data_loader_list(conf['data_folder_test_a'], conf['data_list_test_a'], batch_size, False,
                                 new_size_a, new_size_a, new_size_a, num_workers, True, is_data_A=True))
        train_loader_b.append(
            get_data_loader_list(conf['data_folder_train_b'], conf['data_list_train_b'], batch_size, True,
                                 new_size_b, height, width, num_workers, True, is_data_A=False))
        test_loader_b.append(
            get_data_loader_list(conf['data_folder_test_b'], conf['data_list_test_b'], batch_size, False,
...
def get_data_loader_folder(input_folder, batch_size, train, new_size=None,
    ...
    if config is not None:
        if config['do_RandomResizedCrop']:
            ...
            transform_list = [transforms.RandomResizedCrop(size=new_size,
    ...
    if config is not None:
        if is_data_A is not None:
            if config['do_ColorJitter_A'] and is_data_A:
                transform_list = [transforms.ColorJitter(brightness=config['ColorJitter_brightness'],
            ...
            if config['do_ColorJitter']:
                transform_list = [transforms.ColorJitter(brightness=0.1, contrast=0.1,
                                                         saturation=0.1, hue=config['ColorJitter_hue'])] \
                                 + transform_list if train else transform_list
    ...
    if config is not None:
        if config['do_RandomRotation']:
            transform_list = [transforms.RandomRotation(degrees=config['RandomRotation_degree'], expand=False)]\
                             + transform_list if train else transform_list

    if config is not None:
        if config['do_RandomAffine']:
            config['RandomAffine_translate_w']
            transform_list = [transforms.RandomAffine(translate=(config['RandomAffine_translate_h'],
                                                                 config['RandomAffine_translate_w']),
    ...
    if config is not None:
        if config['do_RandomPerspective']:
            transform_list = [transforms.RandomPerspective(distortion_scale=0.35,
                                                           p=0.5)] + transform_list if train else transform_list
    ...
    if ratio_1_to_2 is None:
        ...
        dataset = ImageFolder_with_subfolders(root1=input_folder_1, root2=input_folder_2, ratio_1_to_2=ratio_1_to_2,
                                              transform=transform)
...
def __write_images(image_outputs, display_image_num, file_name):
    if image_outputs[0].shape[1] > 3:
        ...
        image_outputs = [images[:, rand_disp_index_s, :, :] if images.shape[1] > 3 else images for images in
                         image_outputs]  # TODO tmp
    else:
...
def write_2images(image_outputs, display_image_num, image_directory, postfix, do_a2b=True, do_b2a=True):
    ...
    if do_a2b:
        gen_a2b_im = __write_images(image_outputs[0:n // 2], display_image_num,
                                    '%s/gen_a2b_%s.jpg' % (image_directory, postfix)) if do_a2b else None
    if do_b2a:
        gen_b2a_im = __write_images(image_outputs[n // 2:n], display_image_num,
                                    '%s/gen_b2a_%s.jpg' % (image_directory, postfix)) if do_b2a else None
...
def write_loss(iterations, trainer, train_writer):
    members = [attr for attr in dir(trainer)
               if not callable(getattr(trainer, attr)) and not attr.startswith("__") and (
                       'loss' in attr or 'grad' in attr or 'conf' in attr or 'nwd' in attr or 'do' in attr)]
    for m in members:
