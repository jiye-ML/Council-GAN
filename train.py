...
parser.add_argument('--config', type=str, default='configs/anime2face_council_folder.yaml',
...
try:
    train_display_images_a = torch.stack(
        [train_loader_a[0].dataset[np.random.randint(train_loader_a[0].__len__())] for _ in range(display_size)]).cuda(
        config['cuda_device'])
except:
    train_display_images_a = torch.stack(
        [train_loader_a[0].dataset[np.random.randint(train_loader_a[0].__len__())] for _ in range(display_size)]).cuda(
...
try:
    train_display_images_b = torch.stack(
        [train_loader_b[0].dataset[np.random.randint(train_loader_b[0].__len__())] for _ in range(display_size)]).cuda(
        config['cuda_device'])
except:
    train_display_images_b = torch.stack(
        [train_loader_b[0].dataset[np.random.randint(train_loader_b[0].__len__())] for _ in range(display_size)]).cuda(
...
try:
    test_display_images_a = torch.stack(
        [test_loader_a[0].dataset[np.random.randint(test_loader_a[0].__len__())] for _ in range(display_size)]).cuda(
        config['cuda_device'])
except:
...
try:
    test_display_images_b = torch.stack(
        [test_loader_b[0].dataset[np.random.randint(test_loader_b[0].__len__())] for _ in range(display_size)]).cuda(
        config['cuda_device'])
except:
    test_display_images_b = torch.stack(
        [test_loader_b[0].dataset[np.random.randint(test_loader_b[0].__len__())] for _ in range(display_size)]).cuda(
...
shutil.copy(opts.config, config_backup_folder)
...
if config['misc']['start_tensor_board']:
    ...
    t_tensorBoard = threading.Thread(target=launchTensorBoard,
                                     args=([port]))  # launches TensorBoard in a diffrent thread
    t_tensorBoard.start()
    print(colored('tensorboard board launched at http://127.0.0.1:' + str(port), color='yellow',
                  attrs=['underline', 'bold', 'blink', 'reverse']))
...
if config['misc']['do_telegram_report']:
    ...
    if not os.path.exists(confidential_yaml_file_path):
        ...
        in_ = input(colored(
            'do_telegram_report is set to True If you would like to set up telegram press Enter. If you just Want to continue write \"NO\": \n'))
        if in_.upper() != 'NO':
    ...
    if in_.upper() != 'NO':
        ...
        def telegram_command(update, context):
            ...
            context.bot.sendMessage(update.message.chat_id,
                                    text='enter chat_id in to: ' + confidential_yaml_file_path + ' as:')
        ...
        while confidential_conf['chat_id'] == 'xxxx':
            print(colored(
                'CHAT ID is not defined send your telegram bot a random message to get your chat id, then enter it into the file:' + confidential_yaml_file_path,
                color='red', attrs=['underline', 'bold', 'blink', 'reverse']))
            print('==== You can turn telegram report OFF from the config.yaml file ====')
            input('when you are done press Enter')
            confidential_conf = get_config(confidential_yaml_file_path)
        ...
        def telegram_bot_send_message(bot_message):
            try:
                telegram_bot.send_message(chat_id=confidential_conf['chat_id'],
                                          text=config['misc']['telegram_report_add_prefix'] + bot_message)
            except:
        ...
        def telegram_bot_send_photo(bot_image, caption=None):
            try:
                telegram_bot.send_photo(chat_id=confidential_conf['chat_id'], photo=bot_image,
                                        caption=config['misc']['telegram_report_add_prefix'] + caption)
            except:
        ...
        def telegram_bot_send_document(bot_document_path, filename=None):
            try:
                telegram_bot.send_document(chat_id=confidential_conf['chat_id'], document=open(bot_document_path, 'rb'),
                                           filename=config['misc']['telegram_report_add_prefix'] + filename)
            except:
...
def test_fid(dataset1, dataset2, iteration, train_writer, name, m1=None, s1=None, retun_m1_s1=False, batch_size=10,
             dims=2048, cuda=True):
    ...
    try:
        ...
        print(colored('iteration: ' + str(iteration) + ' ,' + name + ' aprox FID: ' + str(fid_value), color='green',
                      attrs=['underline', 'bold', 'blink', 'reverse']))
    ...
    try:
        ...
        while True:
            ...
            for it, (images_a, images_b) in enumerate(zip(tmp_train_loader_a, tmp_train_loader_b)):
                images_a, images_b = images_a.cuda(config['cuda_device']).detach(), images_b.cuda(
                    config['cuda_device']).detach()

                print(
                    "Iteration: " + str(iterations + 1) + "/" + str(max_iter) + " Elapsed time " + str(time.time() - t)[
                                                                                                   :5])
                t = time.time()
                ...
                numberOf_dis_relative_iteration = config['dis']['numberOf_dis_relative_iteration'] if config['dis'][
                                                                                                          'numberOf_dis_relative_iteration'] > 0 else 1
                ...
                if config['council']['numberOfCouncil_dis_relative_iteration'] > 0:
                    trainer.dis_council_update(images_a, images_b,
                                               config)  # the multiple iterating happens inside dis_council_update
                ...
                if config['misc']['do_test_Fid'] and (iterations + 1) % config['misc']['test_Fid_iter'] == 0:
                    ...
                    if config['do_a2b']:
                        ...
                        if m1_1_a2b is None or s1_1_a2b is None:
                            ...
                            m1_1_a2b, s1_1_a2b = test_fid(dataset_for_fid_B, tmp_path_im_a2b, iterations, train_writer,
                                                          'B',
                                                          retun_m1_s1=True, batch_size=10)
                            ...
                            _ = test_fid(dataset_for_fid_B, tmp_path_im_a2b, iterations, train_writer, 'B', m1_1_a2b,
                                         s1_1_a2b)
                    ...
                    if config['do_b2a']:
                        ...
                        if m1_1_b2a is None or s1_1_b2a is None:
                            print('fid test initialization')
                            m1_1_b2a, s1_1_b2a = test_fid(dataset_for_fid_A, tmp_path_im_b2a, iterations, train_writer,
                                                          'A',
                                                          retun_m1_s1=True, batch_size=10)
                            ...
                            _ = test_fid(dataset_for_fid_A, tmp_path_im_b2a, iterations, train_writer, 'A', m1_1_b2a,
                                         s1_1_b2a)
                ...
                if (iterations + 1) % config['image_save_iter'] == 0:
                    ...
                    if config['do_a2b']:
                        ...
                        if config['misc']['do_telegram_report']:
                            ...
                            telegram_bot_send_document(
                                os.path.join(image_directory, 'gen_a2b_' + 'train_%08d' % (iterations + 1) + '.jpg'),
                                filename='train_gen_a2b_im-iteration: ' + str(iterations) + '.jpg')
                            telegram_bot_send_document(
                                os.path.join(image_directory, 'gen_a2b_' + 'test_%08d' % (iterations + 1) + '.jpg'),
                                filename='test_gen_a2b_im-iteration: ' + str(iterations) + '.jpg')
                    ...
                    if config['do_b2a']:
                        ...
                        if config['misc']['do_telegram_report']:
                            telegram_bot_send_message('iteration: ' + str(iterations))
                            telegram_bot_send_document(
                                os.path.join(image_directory, 'gen_b2a_' + 'train_%08d' % (iterations + 1) + '.jpg'),
                                filename='train_gen_b2a_im-iteration: ' + str(iterations) + '.jpg')
                            telegram_bot_send_document(
                                os.path.join(image_directory, 'gen_b2a_' + 'test_%08d' % (iterations + 1) + '.jpg'),
                                filename='test_gen_b2a_im-iteration: ' + str(iterations) + '.jpg')
                ...
                if (iterations + 1) % config['snapshot_save_iter'] == 0:
                    try:
                        ...
                        print(
                            '================================= Faild to save check avileble memory =================================')
                        print(e)
                        if config['misc']['do_telegram_report']:
